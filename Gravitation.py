...
s = []
for file in files:
	rdbtn = Radiobutton(root,command=open_file,text=file,variable=option_R,value=file,background='#403f3f',foreground='#d3d7e0')
	rdbtn.place(x=335,y=y)
	s += [rdbtn]
	y+=20
backgroundlbl=Label(root,background='#2f3136',width=71,height=4)
backgroundlbl.place(x=0,y=0)
namelbl = Label(root,text='Gravitation',background='#2f3136',foreground='#d3d7e0',font=('Arial Bold',40))
